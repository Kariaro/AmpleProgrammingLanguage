...
public final class ValueTest {
	...
	public void testIntegerAddSubMul() {
		...
		for (Value.Type type : INTEGER_TYPES) {
			for (int i = 0; i < INTEGER_ADD.length; i += 3) {
				...
				d = a.copy().add(b);
				Assertions.assertTrue(d.sub(c).isZero(), "%s + %s = %s (expected: %s)".formatted(a, b, d, c));
			}
	...
	public void testIntegerDiv() {
		...
		for (int i = 0; i < UNSIGNED_DIV_REM.length; i += 4) {
			...
			e = a.copy().div(b);
			Assertions.assertTrue(e.sub(c).isZero(), "unsigned: %s / %s = %s (expected: %s)".formatted(a, b, e, c));
			e = a.copy().remainder(b);
			Assertions.assertTrue(e.sub(d).isZero(), "unsigned: %s %% %s = %s (expected: %s)".formatted(a, b, e, d));
		}
		...
		for (int i = 0; i < SIGNED_DIV_REM.length; i += 4) {
			...
			e = a.copy().div(b);
			Assertions.assertTrue(e.sub(c).isZero(), "signed: %s / %s = %s (expected: %s)".formatted(a, b, e, c));
			e = a.copy().remainder(b);
			Assertions.assertTrue(e.sub(d).isZero(), "signed: %s %% %s = %s (expected: %s)".formatted(a, b, e, d));
		}
	}
	...
	public void testIntegerCompare() {
		...
		for (int i = 0; i < INTEGER_COMPARE.length; i += 4) {
			...
			Assertions.assertEquals(d, c, "unsigned: %s <=> %s = %s (expected: %s)".formatted(a, b, d, c));
		}
		...
		for (int i = 0; i < INTEGER_COMPARE.length; i += 4) {
			...
			int d = a.copy().compare(b);
			Assertions.assertEquals(d, c, "signed: %s <=> %s = %s (expected: %s)".formatted(a, b, d, c));
		}
	}
	
	@Test
	public void testToString() {
		Assertions.assertEquals("-7fedc27892578926", Value.valueOf(0x80123d87_6da876daL, 64, Value.Type.SIGNED).toString(16));
		Assertions.assertEquals("80123d876da876da", Value.valueOf(0x80123d87_6da876daL, 64, Value.Type.UNSIGNED).toString(16));
	}
}
