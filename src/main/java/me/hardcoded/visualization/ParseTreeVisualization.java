		...
		if (obj instanceof Stat stat) {
			return switch (stat.getTreeType()) {
				...
				case GOTO -> {
					...
					yield List.of(s.getReference());
				}
				case IF -> {
					IfStat s = (IfStat)stat;
					yield List.of(s.getCondition(), s.getBody(), s.getElseBody());
				}
				case LABEL -> {
					LabelStat s = (LabelStat)stat;
					yield List.of(s.getReference());
				}
				case PROG -> {
				...
				case BINARY -> {
					...
					yield List.of(e.getLeft(), e.getOperation(), e.getRight());
				}
				case CALL -> {
					CallExpr e = (CallExpr)stat;
					yield List.of(e.getCaller(), e.getParameters());
				}
				case CAST -> {
				...
				case STR -> List.of(((StrExpr)stat).getValue());
				case UNARY -> {
