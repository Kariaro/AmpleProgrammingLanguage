...
public final class InstSourceCodeVisualization extends Visualization implements VisualizationListener {
	...
	private TokenLike getToken(Position pos) {
		List<TokenLike> tokens = sourceCode;
		if (tokens != null) {
			// TODO: This could use binary search
			for (var token : tokens) {
				if (SyntaxUtils.syntaxIntersect(token.value.syntaxPosition, pos)) {
					return token;
				}
			}
		}
	...
	private class LocalPanel extends JPanel {
		...
		public void paint(Graphics gr) {
			...
			if (tokens != null) {
				...
				for (TokenLike token : sourceCode) {
					paintToken(g, pointer, token);
				}
			}
		}
		
		public void paintToken(Graphics2D g, Pointer pointer, TokenLike token) {
			ISyntaxPos pos = token.value.syntaxPosition;
			Position startPos = token.document;
			...
			Color color = switch (token.value.type) {
