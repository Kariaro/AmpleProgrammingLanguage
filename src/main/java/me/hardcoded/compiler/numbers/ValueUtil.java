...
class ValueUtil {
	...
	static int[] add(int[] dst, int[] x, long value) {
		...
		for (int i = 0, len = dst.length; i < len; i++) {
			long a = x[i] & 0xffffffffL;
			long b = i == 0 ? (value & 0xffffffffL) : (i == 1 ? (value >>> 32) : 0);
			long c;
...
//				);
	...
	static int[] set(int[] x, int value) {
		if (value < 0) {
			Arrays.fill(x, -1);
		} else {
			Arrays.fill(x, 0);
...
//			// We can move the array over one step
	...
	static int[] floatSub(int[] dst, int[] x, int[] y, int exponentBits, int mantissaBits) {
		// Truth table of sign
	 ...
	 */
	static int[] floatAdd(int[] dst, int[] x, int[] y, int exponentBits, int mantissaBits) {
		// Truth table of sign
		// : x : y : expression : action
		// | 0 | 0 | x + y      | add
		// | 0 | 1 | x - y      | sub
