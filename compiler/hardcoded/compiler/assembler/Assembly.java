...
//	opr("JMPF", opcode(0xFF), flags("e5"), OprTy.Evqp),							// JMPF			r/m16/32/64
	...
	// Intel 64 and IA-32 manual
	// A   : The register is encoded in the opcode.
	// B   : ....
	// C   : The reg of ModR/M selects a control register.
	// D   : The reg of ModR/M selects a debug register.
	// E   : TODO
	// F   : EFLAGS/RFLAGS register.
	// G   : The reg of ModR/M selects a register.
	// H   : ....
	// I   : Immediate value.
	// J   : Relative offset.
	// L   : ....
	// M   : The ModR/M can only be memory.
	// N   : ....
	// O   : TODO
	// P   : ....
	// Q   : ....
	// R   : The r/m of ModR/M selects a register.
	// S   : The reg of ModR/M selects a segment register.
	// U   : ....
	// V   : ....
	// W   : ....
	// X   : Memory addressed by DS:rSI.
	// Y   : Memory addressed by ES:rDI.
	
	// b   : Always byte.
	// c   : byte/word depending on operand-size.
	// d   : Always dword.
	// p   : 32/48/80 bit depending on operand-size.
	// q   : Always qword.
	// v   : word/dword or (qword 64 bit) depending on operand-size.
	// w   : Always word.
	// y   : dword/qword depending on operand-size.
	// z   : word/dword/qword depending on target operand? TODO
	
	// rXX can be either the 16/32 or 64 bit version of the register.
	// eXX can be either 16 or 32 bit register.
	// A register is encoded by its name.
	// When a register is written reg/... it can be changed to the other
	// register with REX.B.
	
	
	
	public static final Assembly ALL;
	...
	static {
		...
		for(Unit unit : units) {
			...
			for(int i = 0; i < array.length; i++) {
				params[i] = OprTy.valueOf(array[i]);
			}
			
			AsmOp op = opr(unit.getMnemonic(), unit.getOpcode(), flags(unit.getFlags()), params);
			list.add(op);
			System.out.println(op.toPlainString());
		}
		
		ALL = new Assembly(list.toArray(new AsmOp[0]));
	}
	...
	private static boolean matches(OprTy type, AsmOpr opr) {
		...
		if(type.hasData()) {
			...
			String cmps = opr.toString().toUpperCase();
